  ...
  .factory('profileService', function profileServiceFactory($rootScope, $timeout, $filter, $log, sjcl, lodash, storageService, bwcService, configService, gettextCatalog, bwcError, uxLanguage, platformInfo, txFormatService, $state) {
    ...
    var addAndBindWalletClient = function(client, opts, cb) {
      ...
      if (!root.profile.addWallet(JSON.parse(client.export())))
        return cb(gettextCatalog.getString('Wallet already in Renrenbit'));
    ...
    root.importWallet = function(str, opts, cb) {
      ...
      if (!str.n) {
        return cb("Backup format not recognized. If you are using a Renrenbit Beta backup and version is older than 0.10, please see: https://github.com/bitpay/copay/issues/4730#issuecomment-244522614");
      }
